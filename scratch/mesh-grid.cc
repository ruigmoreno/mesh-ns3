...
private:
  int       m_xSize;
  int       m_ySize;
  int       m_standardPhy;          // set phy layer
  // int       m_nbNodes;
  int       m_numFlows;
  int       m_seed;
  double    m_step;
...
MeshJournal::Configure (int argc, char *argv[])
{
  ...
  cmd.AddValue ("phy",   "phy Layer [1]", m_standardPhy);
...
MeshJournal::CreateNodes ()
{
  ...
  std::cout << "m_standardPhy: " << m_standardPhy << std::endl;
  std::cout << "Standard WiFi is " << getEnumFromString(m_standardPhy) << std::endl;
...
MeshJournal::InstallApplication ()
{
  ...
  // std::cerr << Seconds(m_totalTime-1) << std::endl;
  int64x64_t _time = Seconds(m_totalTime-1).To(Time::NS);
  // std::cerr << Seconds(_time) << std::endl;
  clientApps.Stop ( Seconds (_time) );
  */
}
