#!/usr/bin/env python
#-*- coding:utf-8 -*-
import os, cairo, lxml, lxml.etree, glob
from math import pi
...
def main():
    ...
    surf = cairo.SVGSurface( svg_dir+"graph.svg", IMG_W, IMG_H )
...
for filename in glob.iglob(PATH_SCENARIO + '**/**/logMeshSimulationDiscovery.txt', recursive=True):
    dir = filename.split('logMeshSimulationDiscovery.txt')
    svg_dir = dir[0]
    logMeshFile = filename.split('/')[-1]
    xml_dir = svg_dir + 'report/'
    main()
